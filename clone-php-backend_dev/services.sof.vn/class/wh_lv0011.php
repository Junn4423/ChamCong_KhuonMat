...
///////////
	public $DefaultFieldList="lv001,lv002,lv102,lv103,lv104,lv003,lv088,lv089,lv004,lv005,lv198,lv015,lv016,lv008,lv017";
	//public $DefaultFieldList="lv001,lv002,lv102,lv103,lv104,lv003,lv088,lv004,lv005,lv008,lv009,lv014,lv015,lv016,lv017,lv109";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv088"=>'89',"lv089"=>"90","lv099"=>"100","lv100"=>"101","lv102"=>"103","lv103"=>"104","lv104"=>"105","lv109"=>"110","lv198"=>"199");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"20","lv005"=>"21","lv006"=>"10","lv007"=>"0","lv008"=>"10","lv009"=>"21","lv010"=>"10","lv011"=>"10","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"22","lv017"=>"10","lv088"=>"0","lv099"=>"0","lv100"=>"10","lv102"=>"0","lv103"=>"0","lv104"=>"0","lv109"=>"4","lv198"=>"0");	
	...
	function __construct($vCheckAdmin,$vUserID,$vright,$vSkipAuth = false)
	{
		...
		$vlv909=$this->Get_User($_SESSION['ERPSOFV2RUserID'],'lv909');		
		if($vlv909==0) 
		{
			$this->ArrView['lv008']='3';
			$this->ArrView['lv017']='3';			
		}
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into wh_lv0011 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv066) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014','$this->lv015',now(),'$this->lv017','$this->lv018','$this->lv019','$this->lv020',now())";
		echo $lvsql;
		$vReturn= db_query($lvsql);
		if($vReturn){
	...
	function LV_InsertTempSLPBH($vInvoiceID,$vlv002,$vWarehourID)
	{
		$lvsql="select '$vInvoiceID' lv002,A.lv003,A.lv051 lv004,A.lv052 lv005,concat(CURDATE(),' ',CURTIME()) lv016 from cr_lv0276 A inner join sl_lv0007 E on A.lv003=E.lv001  where A.lv002='$vlv002' and E.lv098<>1";
		$vReturn= db_query($lvsql);
		while($vrow=db_fetch_array($vReturn))
		{
			//Mã xuat kho
			...
			$this->lv005=$vrow['lv005'];
			//SL2
			$this->lv006=0;//$vrow['lv006'];
			//DV2
			$this->lv007=0;//$vrow['lv007'];
			//Gía
			$this->lv008=0;//$vrow['lv008'];
			//VNĐ
			$this->lv009=0;//$vrow['lv009'];
			$this->lv010=0;//$vrow['lv010'];
			$this->lv011=0;//$vrow['lv011'];
			$this->lv012=0;//$vrow['lv012'];
			$this->lv013=0;//$vrow['lv013'];
			/////Lot
			$this->lv014=$vrow['lv014'];
			$this->lv015=$vrow['lv015'];
			$this->lv016=$vrow['lv016'];
			...
			if($this->lv004>0)
			{
				$this->LV_Insert_FI_LI_FO($vTypeWH,$vWarehourID,$this->lv003);
			}
		}
		return $vReturn;
	}
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM wh_lv0011  WHERE wh_lv0011.lv001 IN ($lvarr) and lv013<>'1' and (select lv007 from wh_lv0010 B where  B.lv001= wh_lv0011.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) {
	...
	{
		$sqlD = "SELECT A.lv001 lv001,A.lv002 lv002,A.lv003 lv003,A.lv004 lv004,A.lv006 lv006,B.lv002 lvb002 FROM wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 WHERE A.lv001 IN ($lvarr) and lv013<>'1' and (select lv007 from wh_lv0010 B where  B.lv001= A.lv002)<=0;";
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,BB.lv005 lv102,BB.lv006 lv103,A.lv003 lv088,A.lv003 lv089,IF(ISNULL(CC.lv016),BB.lv006,CC.lv016) lv099,BB.lv009 lv109 FROM wh_lv0011 A inner join wh_lv0010 BB on A.lv002=BB.lv001 left join sl_lv0013 CC on BB.lv006=CC.lv001 and BB.lv005 in ('BANHANG','TRAHANG')  WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";		
		$vorder=$curRow;
		...
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv004-->",$this->FormatView($vtotalnum,(int)$this->ArrView['lv004']),$strF);
		$strF=str_replace("<!--lv017-->",$this->FormatView($vsumamount,(int)$this->ArrView['lv017']),$strF);
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT A.*,BB.lv005 lv102,BB.lv006 lv103,BB.lv005 lv102,BB.lv006 lv103,A.lv003 lv088,A.lv003 lv089,IF(ISNULL(CC.lv016),BB.lv006,CC.lv016) lv099,BB.lv009 lv109 FROM wh_lv0011 A inner join wh_lv0010 BB on A.lv002=BB.lv001 left join sl_lv0013 CC on BB.lv006=CC.lv001 and BB.lv005 in ('BANHANG','TRAHANG')  WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,IF(lv017='',lv002,lv017) lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0007";
				//$lvopt=0;
				break;
				/*
			case 'lv003':
	...
	public function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			/*case 'lv003':
				$vsql="select lv001,IF(lv017='',lv002,lv017) lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0007 where lv001='$vSelectID'";
				//$lvopt=0;
				break;*/
			case 'lv088':
