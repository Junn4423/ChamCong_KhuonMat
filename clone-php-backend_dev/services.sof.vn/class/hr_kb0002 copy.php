...
class   hr_kb0002 extends lv_controler
{
	...
	// Thêm bản ghi mới
	function KB_Insert($maNhanVien, $viTri, $trangThai, $hinhAnh, $loaiChamCong)
	{
		...
		$insertSQL = "INSERT INTO hr_kb0002 
                        (lv002, lv003, lv004, lv005, lv006, lv007, lv013, lv014, lv020, lv021, lv046,lv015) 
                    VALUES (
                        '" . addslashes($maNhanVien) . "',
                        NOW(),
                        " . $loaiChamCong . ",
                        '" . addslashes($viTri) . "',
                        " . $trangThai . ",
		...
		$newId = 0; // Khởi tạo giá trị mặc định
		if ($resultInsert) {
			$newId = sof_insert_id();
		}

		// Sửa lại giá trị trả về để bao gồm cả lv001 (ID mới)
		return [
			'success'      => $resultInsert,
			'loaiChamCong' => $loaiChamCong,
			'lv001'        => $newId // Thêm ID mới vào đây
		];
	}
