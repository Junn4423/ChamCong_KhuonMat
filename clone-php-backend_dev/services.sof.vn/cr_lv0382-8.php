...
class   cr_lv0382 extends lv_controler
{
	...
	public $lv006=null;
///////////
	public $DefaultFieldList="lv005,lv009,lv010";	
////////////////////GetDate
...
////////////
	...
	var $ArrViewEnter=array("lv005"=>"11","lv006"=>"89","lv007"=>"33","lv008"=>"99","lv009"=>"-1","lv010"=>"-1");
	var $Tables=array();
	...
	function LV_UpdateAutoTemp($vKetQua,$vViTri,$vHinh)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0382_temp.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_UpdateAuto($vKetQua,$vViTri,$vHinh)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0382.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Insert()
	{
		if($this->isAdd==0) return false;
		$lvsql="insert into cr_lv0382 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->LV_UserID',now())";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
	...
	function LV_Aproval($lvarr)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0382.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0382.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	if($this->isDel==0) return false;
		...
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'cr_lv0382.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	////////GetFile Attachment/////////
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM cr_lv0382 WHERE lv007=0  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
				...
				switch($lstArr[$i])
					{
						...
						default:
							if(($this->isEdit==1)) 
							{
								...
								$lvTdTextBox="<input class='txtenterquick' type=\"textbox\" value=\"".$vGiaTri."\" @03 onfocus=\"if(this.value=='') this.value='".$vGiaTri."'\" onblur=\"UpdateText(this,'".$vrow['lv001']."',$vSoTT)\" style=\"min-width:120px;width:100%;text-align:center\" tabindex=\"2\"  maxlength=\"100\" onKeyPress=\"return CheckKey(event,7)\"/>";
