...
$couchHost = getenv('COUCHDB_HOST') ? getenv('COUCHDB_HOST') : '192.168.1.20';
$couchPort = getenv('COUCHDB_PORT') ? getenv('COUCHDB_PORT') : '5984';
$couchUser = getenv('COUCHDB_USER') ? getenv('COUCHDB_USER') : 'admin';
$couchPass = getenv('COUCHDB_PASS') ? getenv('COUCHDB_PASS') : 'rootsof';
$couchDB = getenv('COUCHDB_DATABASE') ? getenv('COUCHDB_DATABASE') : 'couchdb';
...
function saveToken($vUserName, $vToken, $vDeviceType,$couchDBOk='')
{
  ...
  switch (strtolower($vDeviceType)) {
    ...
    case 'chamcongdes':
      ...
      $userData['lv498'] = date('Y-m-d H:i:s'); // Token date Desktop Cham Cong
      $userData['lv499'] = 0; // Allow Desktop Cham Cong login
      break;
    case 'chamcongapp':
      $userData['lv597'] = $vToken; // Token App Cham Cong
      $userData['lv598'] = date('Y-m-d H:i:s'); // Token date App Cham Cong
      $userData['lv599'] = 0; // Allow App Cham Cong login
      break;
    default:
