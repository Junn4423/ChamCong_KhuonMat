<?php
/////////////coding cr_lv0153///////////////
class   cr_lv0153 extends lv_controler
{
	public $lv001 = null;
	public $lv002 = null;
	public $lv003 = null;
	...
	function LV_GetStaffDep()
	{
		$lvsql = "select A.lv001 from hr_lv0020 A inner join hr_lv0002 B on A.lv029=B.lv001 and B.lv198='$this->LV_UserID'";
	...
	function LV_QuetLaiKho($lvarr)
	{
		$lvsql = "select lv001 from  cr_lv0150 Where cr_lv0150.lv001 IN ($lvarr)  and lv007=1 and lv027=1";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
			$this->mocr_lv0151->LV_QuetKhoDNCVT($vrow['lv001']);
		}
	}
	function LV_Aproval($lvarr)
	{
		// if($this->isApr==0) return false;
		$lvsql = "Update cr_lv0150 set lv027=2  WHERE cr_lv0150.lv001 IN ($lvarr)  and lv007=1 and lv027=1";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	function LV_SetHistory($vFun, $vLongTermID)
	{
		...
		switch ($vFun) {
			case 'Apr':
				$vTitle = "ĐN cấp VT quản lý duyệt!";
				break;
			case 'UnApr':
				$vTitle = "ĐN cấp VT quản lý trả lại!";
				break;
			default:
				break;
		}
		//cr_lv0313
		if ($vTitle != '') {
			$lvsql = "insert into cr_lv0313 (lv002,lv003,lv004,lv005,lv006,lv007,lv009) values('" . $vLongTermID . "','" . sof_escape_string($vTitle) . "','$this->LV_UserID',now(),'$vFun',1,'" . sof_escape_string($this->Remark) . "')";
			$vReturn = db_query($lvsql);
			if ($vReturn) {
	...
	function LV_CheckDetail($vEmpID, $vWhID, $vNow, &$vGetKQ = '')
	{
		...
		$vReturn = true;
		$lvsql = "select A.lv001,A.lv003 ItemID,A.lv004 SLItem,(select sum(A1.lv004) from ts_lv0009 A1 where A1.lv003=A.lv003 and A1.lv002 IN (select A11.lv001 from ts_lv0008 A11 Where A11.lv009<'$vDateStart' and A11.lv002='$vWhID')) ReReceiptQty,(select sum(A1.lv004) from ts_lv0009 A1 where A1.lv003=A.lv003 and A1.lv002 IN (select A11.lv001 from ts_lv0008 A11 Where A11.lv009>='$vDateStart' and A11.lv009<='$vDateEnd' and A11.lv002='$vWhID')) InReceiptQty,(select sum(A2.lv004) from ts_lv0011 A2 where A2.lv003=A.lv003 and A2.lv002 IN (select A21.lv001 from cr_lv0150 A21 Where A21.lv009<'$vDateStart' and A21.lv002='$vWhID')) ReOutlv004 ,(select sum(A2.lv004) from ts_lv0011 A2 where A2.lv003=A.lv003 and A2.lv002 IN (select A21.lv001 from cr_lv0150 A21 Where A21.lv009>='$vDateStart' and A21.lv009<='$vDateEnd' and A21.lv002='$vWhID')) InOutlv004 from ts_lv0031 A where lv007=1 and lv027=1 and A.lv002='$vEmpID'";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
	...
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update cr_lv0150 set lv007=0,lv027=0,lv028=0  WHERE cr_lv0150.lv001 IN ($lvarr) and lv007=1 and lv027=1 ";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	protected function GetCondition()
	{
		...
		if ($this->ListStaffValue != '') {
			$strCondi = $strCondi . " and lv003 in ($this->ListStaffValue)";
		}
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM cr_lv0150 WHERE lv007=1 and lv027=1 " . $this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney,sum(PM.lv005) discount from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004,sum(A.lv004*A.lv008*A.lv011/100) lv005 from ts_lv0011 A inner join cr_lv0150 B on A.lv002=B.lv001  where lv007=1 and lv027=1 and B.lv001='$vContractID' )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
	...
	function LV_BuilList($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE lv007=1 and lv027=1  " . $this->GetCondition() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		...
		while ($vrow = db_fetch_array($bResult)) {
			...
			if ($vrow['lv028'] == 0) {
				$this->mocr_lv0151->LV_QuetKhoDNCVT($vrow['lv001']);
				$lvsql = "Update cr_lv0150 set lv028=1 where  lv001='" . $vrow['lv001'] . "' AND lv028=0;";
				$vReturn = db_query($lvsql);
			}
			$vlineamount2 = 0;
				...
				switch ($lstArr[$i]) {
					...
					case 'lv116':
						if ($vrow['lv116'] == '') {
							...
							$vChucNang = $vChucNang . '
							<td><img Title="Báo cáo bảo hành" style="cursor:pointer;width:25px;padding:5px;" onclick="Rpt_BaoHanh(\'' . ($vrow['lv116']) . '\')" alt="NoImg" src="../images/icon/detail.png" align="middle" border="0" name="new" class="lviconimg"></td>
							';
							$vChucNang = $vChucNang . "</tr></table>";
							$vTemp = str_replace("@02", $vChucNang, $this->Align($lvTd, (int)$this->ArrView[$lstArr[$i]]));
						}
	...
	function ListFieldExport($lvFrom, $lvList, $maxRows)
	{
		...
		{
			window.open('" . $this->Dir . "cr_lv0153/?lang=" . $this->lang . "&childfunc='+value+'&ID=" . base64_encode($this->lv002) . "','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReport($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE lv007=1 and lv027=1  " . $this->RptCondition . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function LV_BuilListReportMini($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvDateSort)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE lv007=1 and lv027=1 and lv009 like '$lvDateSort%' " . $this->GetConditionMini() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function LV_BuilListReportOther($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE lv007=1 and lv027=1  " . $this->GetConditionRpt() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function MB_LV_UnAproval($lvarr)
	{
		// if ($this->isUnApr == 0) return false;
		$lvsql = "Update cr_lv0150 set lv007=0,lv027=0,lv028=0,lv030=0  WHERE cr_lv0150.lv001 IN ($lvarr) ";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
			$this->InsertLogOperation($this->DateCurrent, 'cr_lv0150.unapproval', sof_escape_string($lvsql));
			$this->LV_SetHistoryArr('UnApr', $lvarr);
