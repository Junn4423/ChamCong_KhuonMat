<?php
/////////////coding cr_lv0384///////////////
class   cr_lv0384 extends lv_controler
{
	public $lv001 = null;
	public $lv002 = null;
	public $lv003 = null;
	...
	public $lastId = null;
	///////////
	public $DefaultFieldList = "lv003,lv004,lv005,lv006,lv008,lv009,lv010";
	////////////////////GetDate
	public $DateCurrent = "1900-01-01";
	public $Count = null;
	public $paging = null;
	public $lang = null;
	protected $objhelp = 'cr_lv0384';
	public $Dir = "";
	////////////
	var $ArrOther = array();
	var $ArrPush = array();
	...
	var $Tables = array('lv008' => 'hr_lv0020');
	...
	{
		$lvsql = "select lv007,lv027 from cr_lv0150 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql = "select * from  cr_lv0384 Where lv001='$vlv001'";
		$vresult = db_query($lvsql);
		$vrow = db_fetch_array($vresult);
		if ($vrow) {
			$this->lv001 = $vrow['lv001'];
	...
	function LV_LoadUserID($vlv002)
	{
		...
		$lvsql = "select lv001,lv005,lv003 from  cr_lv0384 Where lv002='$vlv002'";
		$vresult = db_query($lvsql);
	...
	function LV_LoadUserArray($vlv002)
	{
		$strReturn = "";
		$strTotal = 0;
		$lvsql = "select * from  cr_lv0384 Where lv002='$vlv002'";
		$vresult = db_query($lvsql);
		return $vresult;
	}
	...
	function LV_LoadStep($vlv007 = 'sovanban', $vlv008 = 'tokhai', $vlv009 = 'bbbg', $vCodeID)
	{
		...
		$lvsql = "select * from  cr_lv0384 Where lv007='$vlv007' and lv008='$vlv008' and lv009='$vlv009'";
		$vresult = db_query($lvsql);
		if ($this->lang == 'VN') $btxoa = "Xóa";
		else $btxoa = "Delete";
		while ($vrow = db_fetch_array($vresult)) {
	...
	function LV_InsertAutoTemp($vViTri, $vHinh)
	{
		...
		if ($vReturn) {
			...
			$this->InsertLogOperation($this->DateCurrent, 'cr_lv0384_temp.insert', sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	function LV_Insert()
	{
		...
		// if ($this->isAdd == 0) return false;
		$lvsql = "insert into cr_lv0384 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->LV_UserID',now())";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	function LV_Update()
	{
		if (!$this->LV_CheckLocked($this->lv002)) return false;
		if ($this->isEdit == 0) return false;
		$lvsql = "Update cr_lv0384 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->LV_UserID',lv010=now() where  lv001='$this->lv001';";
		$vReturn = db_query($lvsql);
		if ($vReturn) $this->InsertLogOperation($this->DateCurrent, 'cr_lv0384.update', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		...
		if ($_SESSION['ERPSOFV2RUserID'] == 'admin')
			$lvsql1 = "select A.lv001,A.lv007 TrangThai,B.lv016 LockTUTT,B.lv028 LockHU from cr_lv0384 A inner join cr_lv0150 B on A.lv002=B.lv001    WHERE A.lv001 IN ($lvarr)";
		else
			$lvsql1 = "select A.lv001,A.lv007 TrangThai,B.lv016 LockTUTT,B.lv028 LockHU from cr_lv0384 A inner join cr_lv0150 B on A.lv002=B.lv001    WHERE A.lv001 IN ($lvarr)";
		...
		while ($vrow = db_fetch_array($bResult)) {
			...
			if ($vIsDel) {
				$lvsql = "DELETE FROM cr_lv0384  WHERE lv001='" . $vrow['lv001'] . "'";
				$vReturn = db_query($lvsql);
				if ($vReturn) {
	...
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update cr_lv0384 set lv006=1  WHERE cr_lv0384.lv001 IN ($lvarr)  ";
		$vReturn = db_query($lvsql);
		if ($vReturn) $this->InsertLogOperation($this->DateCurrent, 'cr_lv0384.approval', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UnAproval($lvarr)
	{
		if ($this->isUnApr == 0) return false;
		$lvsql = "Update cr_lv0384 set lv006=0  WHERE cr_lv0384.lv001 IN ($lvarr)  ";
		$vReturn = db_query($lvsql);
		if ($vReturn) $this->InsertLogOperation($this->DateCurrent, 'cr_lv0384.unapproval', sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_GetForward($AttachFile, $AttachFileHidden)
	{
		...
		for ($i = 0; $i < count($arrFile); $i++) {
			$strTemp = $strTr;
			$this->lv001 = InsertWithCheckExt('cr_lv0384', 'lv001', '', 0);
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM cr_lv0384 WHERE lv007=0 " . $this->GetCondition();
	...
	function LV_BuilList($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM cr_lv0384 WHERE lv007=0  " . $this->GetCondition() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		...
		for ($i = 0; $i < count($lstArr); $i++) {
			...
			switch ($this->ArrViewEnter[$vField]) {
				//browfile
				case 11:
					$lvImg = "<a target='_blank' href='../cr_lv0384/readfiletemp.php?FileID=" . $vrow['lv001'] . "&type=8&size=0'><div style='border-radius:5px;background:#aaa;padding:4px;color:#000;width:40px;text-align:center;'>View</div></a>";
					$vstr = '
						<table valign=\"top\" style=\"width:100%\">
							<tr>
								...
								<div id="framuploadtemp" style="width:116px">
									<iframe  height=24 width="100%" marginheight=0 marginwidth=0 frameborder=0 src="../cr_lv0384/?childdetailfunc=uploadtemp&ViTriUp=8&FileID=' . $vrow['lv001'] . '&lang=' . $this->lang . '"></iframe>
								</td>
								<td align=\"center\">
		...
		while ($vrow = db_fetch_array($bResult)) {
			...
			for ($i = 0; $i < count($lstArr); $i++) {
				if ($lstArr[$i] == 'lv005') {
					$lvImgView = "<a target='_blank' href='../cr_lv0384/readview.php?FileID=" . $vrow['lv001'] . "&type=8&size=0'><div style=\"border-radius:5px;background:#aaa;padding:4px;color:#000;white-space:nowrap\">Xem nhanh</div></a>";
					$lvImg = "<a target='_blank' href='../cr_lv0384/readfile.php?FileID=" . $vrow['lv001'] . "&type=8&size=0'><div style=\"border-radius:5px;background:#aaa;padding:4px;color:#000;\">Download</div></a>";
					...
					<table valign=\"top\" style=\"width:100%\">
						...
						<iframe  height=24 width="100%" marginheight=0 marginwidth=0 frameborder=0 src="../cr_lv0384/?childdetailfunc=upload&ViTriUp=8&FileID=' . $vrow['lv001'] . '&lang=' . $this->lang . '"></iframe>
						</div>
						</td>
					...
					') . '
						...
						<td align=\"center\" nowrap>
							<div style=\"text-align:center\" >' . $lvImgView . '</div>
						</td>
					</tr>
					</table>	
							';
	...
	function ListFieldExport($lvFrom, $lvList, $maxRows)
	{
		...
		{
			window.open('$this->Dir'+'cr_lv0384/?lang=" . $this->lang . "&childdetailfunc='+value+'&ID=" . base64_encode($this->lv002) . "','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReport($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		if ($this->isFull == true) {
			if ($this->MaPhieuID != '')
				$sqlS = "SELECT * FROM cr_lv0384 WHERE lv002='$this->MaPhieuID'";
			else
				$sqlS = "SELECT * FROM cr_lv0384 WHERE  1=1  " . $this->RptCondition . " $strSort LIMIT $curRow, $maxRows";
		} else {
			if ($this->MaPhieuID != '')
				$sqlS = "SELECT * FROM cr_lv0384 WHERE lv007=0 and lv002='$this->MaPhieuID'";
			else
				$sqlS = "SELECT * FROM cr_lv0384 WHERE  lv007=0  " . $this->RptCondition . " $strSort LIMIT $curRow, $maxRows";
		}
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
				...
				if ($lstArr[$i] == 'lv005') {
					...
					<table valign=\"top\" style=\"width:100%\">
						<tr>
							...
							';
					$vTemp = str_replace("@02", $vTempEnter, $this->Align($lvTd, (int)$this->ArrView[$lstArr[$i]]));
				} else {
	...
	private function sqlcondition($vFile, $vSelectID)
	{
		...
		switch ($vFile) {
			case 'lv003':
				$vsql = "select lv001, lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0040 where lv001 in ('HOADON','CTK')";
				break;
			case 'lv006':
	...
	function MB_LoaiTaiLieu()
	{
		$vsql = "select lv001, lv002 from  hr_lv0040 where lv001 in ('HOADON','CTK')";
		return db_query($vsql);
	}
	function MB_LoaiTapTin()
	{
		$vsql = "select lv001,lv002 from  hr_lv0240";
		return db_query($vsql);
	}
	function MB_LoadTaiLieu($maPhieu)
	{
		$vsql = "select * from  cr_lv0384 where lv002 = '$maPhieu'";
		return db_query($vsql);
	}
