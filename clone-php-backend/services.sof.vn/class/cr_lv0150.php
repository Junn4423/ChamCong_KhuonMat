...
/////////////coding cr_lv0150///////////////
class   cr_lv0150 extends lv_controler
{
	public $lv001 = null;
	public $lv002 = null;
	public $lv003 = null;
	...
	///////////
	public $DefaultFieldList = "lv199,lv001,lv029,lv214,lv114,lv115,lv116,lv119,lv112,lv002,lv003,lv829,lv862,lv004,lv009,lv010,lv117,lv118,lv027";
	////////////////////GetDate
	...
	public $paging = null;
	public $lang = null;
	protected $objhelp = 'cr_lv0150';
	////////////
	var $ArrOther = array();
	var $ArrPush = array();
	var $ArrFunc = array();
	var $ArrGet = array("lv001" => "2", "lv002" => "3", "lv003" => "4", "lv004" => "5", "lv005" => "6", "lv006" => "7", "lv007" => "8", "lv008" => "9", "lv009" => "10", "lv010" => "11", "lv011" => "12", "lv012" => "13", "lv013" => "14", "lv014" => "15", "lv023" => "24", "lv027" => "28", "lv099" => "100", "lv106" => "107", "lv114" => "115", "lv115" => "116", "lv116" => "117", "lv119" => "120", "lv117" => "118", "lv118" => "119", "lv199" => "200", "lv214" => "215", "lv112" => "113", "lv829" => "830", "lv862" => "863", "lv029" => "29");
	var $ArrView = array("lv001" => "0", "lv002" => "0", "lv003" => "0", "lv004" => "0", "lv005" => "0", "lv006" => "0", "lv007" => "0", "lv008" => "0", "lv009" => "22", "lv010" => "2", "lv011" => "0", "lv012" => "0", "lv013" => "0", "lv014" => "0", "lv023" => "10", "lv027" => "0", "lv099" => "0", "lv016" => "0", "lv114" => "0", "lv199" => "0", "lv117" => "0", "lv118" => "22", "lv112" => "10", "lv118" => "22");
	...
	{
		$lvsql = "insert into ts_lv0031 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016) select '" . getInfor($_SESSION['ERPSOFV2RUserID'], 2) . "',lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv012,'',lv013,lv014,lv015,lv016 from cr_lv0151 where lv002='$vlv002'";
	...
	function LV_Update()
	{
		...
		$this->lv009 = ($this->lv009 != "") ? recoverdate(($this->lv009), $this->lang) : $this->DateDefault;
		$this->lv010 = ($this->lv010 != "") ? recoverdate(($this->lv010), $this->lang) : $this->DateDefault;
		$lvsql = "Update cr_lv0150 set  lv004='" . sof_escape_string($this->lv004) . "',lv009=concat('$this->lv009',' ',CURRENT_TIME()),lv002='$this->lv002',lv114='$this->lv114' where  lv001='$this->lv001' AND lv007<=0;";
		//$lvsql="Update cr_lv0150 set  lv004='".sof_escape_string($this->lv004)."',lv009=concat('$this->lv009',' ',CURRENT_TIME()),lv002='$this->lv002',lv114='$this->lv114',lv115='$this->lv115',lv116='$this->lv116' where  lv001='$this->lv001' AND lv007<=0;";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	public function GetBuilCheckListDept($vListID, $vID, $vTabIndex, $vTbl, $vFieldView = 'lv002', $vDepID = "")
	{
		...
		if ($_SESSION['ERPSOFV2RUserID'] == 'admin') {
			$vsql = "select A.* from  " . $vTbl . " A inner join wh_lv0034 B on A.lv001=B.lv003 and B.lv002='" . $_SESSION['ERPSOFV2RUserID'] . "'  where A.lv001 in ('KHOVATTU','3') or A.lv001 like 'KHOVATTU%'";
		}
		if ($vDepID == "") {
			$vsql = "select A.* from  " . $vTbl . " A inner join wh_lv0034 B on A.lv001=B.lv003 and B.lv002='" . $_SESSION['ERPSOFV2RUserID'] . "'  where A.lv001 in ('KHOVATTU','3') or A.lv001 like 'KHOVATTU%'";
		} else {
			$vReturn = "'" . str_replace(",", "','", $vDepID) . "'";
			$vsql = "select A.lv001,A.lv003 from  ts_lv0001 A inner join wh_lv0034 B on A.lv001=B.lv003 and B.lv002='" . $_SESSION['ERPSOFV2RUserID'] . "' where (A.lv001 in ($vReturn)) and ((A.lv001 in ('KHOVATTU','3'))  or (A.lv001 like 'KHOVATTU%')) ";
		}
		$vsql = "select A.* from  " . $vTbl . " A inner join wh_lv0034 B on A.lv001=B.lv003 and B.lv002='" . $_SESSION['ERPSOFV2RUserID'] . "'  where A.lv001 in ('KHOVATTU','3') or A.lv001 like 'KHOVATTU%'";
		//echo $vsql;
		$strGetList = "";
	...
	function LV_GetStaffDepTGD($vStaffID)
	{
		if ($this->ArrStaff[$vStaffID][0]) return $this->ArrStaff[$vStaffID][1];
		$this->ArrStaff[$vStaffID][0] = true;
		$lvsql = "select A.lv001 from hr_lv0020 A inner join hr_lv0002 B on A.lv029=B.lv001 and B.lv198 in ('MP001','MP002') and A.lv001='$vStaffID'";
		$lvResult = db_query($lvsql);
		while ($row = db_fetch_array($lvResult)) {
			$this->ArrStaff[$vStaffID][1] = true;
			return 	$this->ArrStaff[$vStaffID][1];
		}
		$this->ArrStaff[$vStaffID][1] = false;
		return 	$this->ArrStaff[$vStaffID][1];
	}
	function LV_Aproval($lvarr)
	{
		// if ($this->isApr == 0) return false;
		$lvsql1 = "select lv001,lv003 NguoiDN,lv004 from cr_lv0150 where cr_lv0150.lv001 IN ($lvarr)  and lv007=0 and lv027=0";
		$bResult = db_query($lvsql1);
		while ($vrow = db_fetch_array($bResult)) {
			if (trim($vrow['lv004']) == '') {
				echo '<font color="red">ĐNVT: ' . $vrow['lv001'] . ' không có mục đích sử dụng. Xin vui lòng nhập mục đích sử dụng!<br/></font>';
			} else {
				$vGiaiTriSet = 'lv007=1,lv027=1';
				$vIsTGD = $this->LV_GetStaffDepTGD($vrow['NguoiDN']);
				if ($vIsTGD) {
					//Thiết lập qua duyệt 
					$vGiaiTriSet = 'lv007=1,lv027=2';
				}
				$lvsql = "Update cr_lv0150 set $vGiaiTriSet  where lv001 ='" . $vrow['lv001'] . "' and lv007=0 and lv027=0";
				$vReturn = db_query($lvsql);
				if ($vReturn) {
					//$this->LV_InsertLocal($lvarr);
					$this->InsertLogOperation($this->DateCurrent, 'cr_lv0150.approval', sof_escape_string($lvsql));
					$this->LV_SetHistoryArr('Apr', $vrow['lv001']);
				}
			}
		}
		return $vReturn;
	...
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM cr_lv0149 WHERE lv002='$this->LV_UserID' ";
	...
	function LV_CheckDetail($vEmpID, $vWhID, $vNow, &$vGetKQ = '')
	{
		...
		$vReturn = true;
		$lvsql = "select A.lv001,A.lv003 ItemID,A.lv004 SLItem,(select sum(A1.lv004) from ts_lv0009 A1 where A1.lv003=A.lv003 and A1.lv002 IN (select A11.lv001 from ts_lv0008 A11 Where A11.lv009<'$vDateStart' and A11.lv002='$vWhID')) ReReceiptQty,(select sum(A1.lv004) from ts_lv0009 A1 where A1.lv003=A.lv003 and A1.lv002 IN (select A11.lv001 from ts_lv0008 A11 Where A11.lv009>='$vDateStart' and A11.lv009<='$vDateEnd' and A11.lv002='$vWhID')) InReceiptQty,(select sum(A2.lv004) from ts_lv0011 A2 where A2.lv003=A.lv003 and A2.lv002 IN (select A21.lv001 from cr_lv0150 A21 Where A21.lv009<'$vDateStart' and A21.lv002='$vWhID')) ReOutlv004 ,(select sum(A2.lv004) from ts_lv0011 A2 where A2.lv003=A.lv003 and A2.lv002 IN (select A21.lv001 from cr_lv0150 A21 Where A21.lv009>='$vDateStart' and A21.lv009<='$vDateEnd' and A21.lv002='$vWhID')) InOutlv004 from ts_lv0031 A where 1=1 and A.lv002='$vEmpID'";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
	...
	{
		$vNow = GetServerDate();
		$lvsql = "select A.* from cr_lv0150 A  where (A.lv010='NOIBO' or (A.lv010='THAYTH' and (A.lv099<>'' OR A.lv099<>NULL))) AND A.lv001 IN ($lvarr) and (select count(*) from ts_lv0008 BB where BB.lv006=A.lv001 and BB.lv005='XUATKHO')<=0";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
			...
			if (db_query($vslq)) {
				...
				if ($vrow['lv010'] == 'THAYTH') {
					$vPXID = InsertWithCheck('cr_lv0150', 'lv001', 'PX-' . getmonth($vNow) . "/" . getyear($vNow) . "-", 4);
					$vslq = "insert into cr_lv0150(lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv099) values('" . $vPXID . "','" . $vrow['lv099'] . "','" . $vrow['lv003'] . "','" . $vrow['lv004'] . "','NHAPKHO','" . $vPNID . "','" . $vrow['lv007'] . "','" . $vrow['lv008'] . "','" . $vrow['lv009'] . "','" . $vrow['lv010'] . "','')";
					if (db_query($vslq)) {
	...
	//History 
	function LV_GetHistory($vDNPCID, $vLevel = '')
	{
		$vArrDuyet = array();
		if ($vLevel != '')
			$vsql = "select lv004,lv005 from  cr_lv0313 where lv002='$vDNPCID' and lv007='$vLevel' and lv008=0 and lv006 in ('Apr') order by lv005 desc";
		else
			$vsql = "select lv004,lv005 from  cr_lv0313 where lv002='$vDNPCID'  and lv008=0 and lv006 in ('Apr') order by lv005 desc";
		$vresult = db_query($vsql);
		while ($vrow = db_fetch_array($vresult)) {
			if ($vArrDuyet[$vrow['lv004']] == null) $vArrDuyet[$vrow['lv004']] = $vrow['lv005'];
		}
		return $vArrDuyet;
	}
	...
	function LV_SetHistory($vFun, $vLongTermID)
	{
		...
		switch ($vFun) {
			case 'Apr':
				$vTitle = "ĐN cấp VT đề xuất lên quản lý duyệt!";
				break;
			case 'UnApr':
				$vTitle = "ĐN cấp VT thì bị huỷ!";
				break;
			default:
				break;
		}
		//cr_lv0313
		if ($vTitle != '') {
			$lvsql = "insert into cr_lv0313 (lv002,lv003,lv004,lv005,lv006,lv007) values('" . $vLongTermID . "','" . sof_escape_string($vTitle) . "','$this->LV_UserID',now(),'$vFun',0)";
			$vReturn = db_query($lvsql);
			if ($vReturn) {
	...
	{
		if ($this->isApr == 0) return false;
		$lvsql = "Update cr_lv0150 set lv007=0,lv027=-1  WHERE cr_lv0150.lv001 IN ($lvarr) and lv007=0 and lv027=0 ";
		$vReturn = db_query($lvsql);
		if ($vReturn) $this->InsertLogOperation($this->DateCurrent, 'cr_lv0150.unapproval', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_InsertTemp()
	{
		...
		$lvsql = "insert into cr_lv0150 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv099,lv114,lv115,lv116,lv117,lv118,lv119,lv029) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$userID','$this->lv008',concat('$this->lv009',' ',CURRENT_TIME()),'$this->lv010','$this->lv011','$this->lv099','$this->lv114','$this->lv115','$this->lv116','$userID',now(),'$this->lv119','$this->lv029')";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
			//$this->LV_InsertOther($this->lv001);
			$this->InsertLogOperation($this->DateCurrent, 'cr_lv0150.insert', sof_escape_string($lvsql));
	...
	function LV_CreateTC($strar)
	{
		...
		foreach ($vArrdonhang as $donhang) {
			$this->LV_LoadID($donhang);
			if ($donhang != "") {
				$vcusid = $this->lv002;
				$this->lvts_lv0003->LV_LoadID($vcusid);
				//$this->lvcr_lv0202=new ts_lv0052($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0052');
				$this->lvcr_lv0202->lv001 = InsertWithCheckFist('cr_lv0202', 'lv001', '/ĐNPC/MP' . substr(getyear($vNow), -2, 2), 4);
				$this->lvcr_lv0202->lv002 = '1';
				$this->lvcr_lv0202->lv003 = 'SUP';
				$this->lvcr_lv0202->lv004 = $this->lvts_lv0003->lv001;
				$this->lvcr_lv0202->lv005 = $this->lvts_lv0003->lv002;
				$this->lvcr_lv0202->lv006 = $this->lvts_lv0003->lv006;
				$this->lvcr_lv0202->lv007 = $this->lv807;
				$this->lvcr_lv0202->lv008 = $this->LV_UserID;
				$this->lvcr_lv0202->lv009 = $this->FormatView($vNow, 2);
				if ($this->lvcr_lv0202->lv010 == '') $this->lvcr_lv0202->lv010 = '1111';
				if ($this->lvcr_lv0202->lv910 == '')  $this->lvcr_lv0202->lv910 = '331';
				$this->lvcr_lv0202->lv011 = 'VND';
				$this->lvcr_lv0202->lv012 = '1';
				$this->lvcr_lv0202->lv013 = $this->lv001;
				$this->lvcr_lv0202->lv014 = $this->FormatView($vInvoiceDate, 2);
				$this->lvcr_lv0202->lv015 = $donhang;
				$this->lvcr_lv0202->lv016 = '';
				$this->lvcr_lv0202->lv018 = $this->lv818;
				$this->lvcr_lv0202->lv022 = 'SOF';
				$this->lvcr_lv0202->lv019 = $this->LV_GetBLMoneyMore($donhang);
				if ($this->lvcr_lv0202->lv019 > 0) $this->lvcr_lv0202->LV_InsertDNCT();
			}
		}
		$this->lv002 = '';
		$this->lv001 = '';
	...
	function LV_CreateNganHang($strar)
	{
		...
		foreach ($vArrdonhang as $donhang) {
			...
			if ($donhang != "") {
				...
				$this->lvcr_lv0202->lv008 = $this->LV_UserID;
				$this->lvcr_lv0202->lv009 = $this->FormatView($vNow, 2);
				if ($this->lvcr_lv0202->lv010 == '') $this->lvcr_lv0202->lv010 = '1121';
				if ($this->lvcr_lv0202->lv910 == '')  $this->lvcr_lv0202->lv910 = '331';
				$this->lvcr_lv0202->lv011 = 'VND';
				$this->lvcr_lv0202->lv012 = '1';
				$this->lvcr_lv0202->lv013 = $this->lv001;
				...
				if ($vMoney == 0)
					...
					$this->lvcr_lv0202->lv019 = $vMoney;
				if ($this->lvcr_lv0202->lv019 > 0) {
					$this->lvcr_lv0202->LV_InsertDNCT();
				}
			}
		}
		$this->lv002 = '';
	...
	function LV_GetBLMoneyMore($vContractID)
	{
		$lvsql = "select sum(PM.lv003) money,sum(PM.lv004) VAT,sum(PM.lv005) thuekhac from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv006*A.lv010/100) lv004,0 lv005 from cr_lv0151 A inner join cr_lv0150 B on A.lv002=B.lv001  where 1=1 and B.lv001='$vContractID' )) PM ";
		$vresult = db_query($lvsql);
		$vrow = db_fetch_array($vresult);
		if ($vrow['VAT'] == 0) {
	...
	protected function GetCondition()
	{
		...
		if (trim($this->TenDuAn) != "") {
			...
			$strCondi = $strCondi . " and lv114 in ($vPrjName)";
		}
		if (trim($this->TenKH) != "") {
		...
		if ($this->lv004 != "") {
			if (!strpos($this->lv004, ',') === false) {
				...
				foreach ($vArrNameCus as $vNameCus) {
					if ($vNameCus != "") {
						if ($strCondi == "")
							$strCondi = " AND ( lv004 = '$vNameCus'";
						else
							$strCondi = $strCondi . " OR lv004 = '$vNameCus'";
					}
				}
				$strCondi = $strCondi . ")";
			} else {
				$strCondi = $strCondi . " and lv004  = '$this->lv004'";
			}
		}
		...
		if ($this->lv027 != "") {
			switch ($this->lv027) {
				case 20:
					$strCondi = $strCondi . " and lv027  = '2' and lv030='0'";
					break;
				case 21:
					$strCondi = $strCondi . " and lv027  = '2' and lv030='1'";
					break;
				default:
		...
		if ($this->lv116 != "") $strCondi = $strCondi . " and lv116  = '$this->lv116'";
		if ($this->lv119 != "") $strCondi = $strCondi . " and lv119  = '$this->lv119'";
		if ($this->lv011 != "") {
			if (!strpos($this->lv011, ',') === false) {
				...
				foreach ($vArrNameCus as $vNameCus) {
					if ($vNameCus != "") {
						if ($strCondi == "")
							$strCondi = " AND ( lv011 = '$vNameCus'";
						else
							$strCondi = $strCondi . " OR lv011 = '$vNameCus'";
					}
				}
				$strCondi = $strCondi . ")";
			} else {
				$strCondi = $strCondi . " and lv011  = '$this->lv002'";
			}
		}
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM cr_lv0150 WHERE 1=1 " . $this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	...
	{
		$lvsql = "select sum(PM.lv003) money,sum(PM.lv004) convertmoney,sum(PM.lv005) discount from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004,sum(A.lv004*A.lv008*A.lv011/100) lv005 from cr_lv0151 A inner join cr_lv0150 B on A.lv002=B.lv001  where 1=1 and B.lv001='$vContractID' )) PM ";
		$vresult = db_query($lvsql);
		$vrow = db_fetch_array($vresult);
	...
	{
		$vResult = '';
		$lvsql = "select B.lv001 from cr_lv0202 B where B.lv013='$vContractID' ";
		$lvResult = db_query($lvsql);
		while ($row = db_fetch_array($lvResult)) {
			if ($vResult == "")
				$vResult = "'" . $row['lv001'] . "'";
	...
	function LV_GetPCMoney($vContractID, $vTKNo = '')
	{
		//GetMoney
		$vListParent = $this->LV_GetInvoiceParent($vContractID);
		$lvsql = "select if(ISNULL(sum(A.lv004)),0,sum(A.lv004)) SumMoney from cr_lv0203 A inner join cr_lv0202 B on A.lv002=B.lv001 WHERE B.lv002=1 and B.lv017=0 and A.lv002 in (" . $vListParent . ") and A.lv005 like '$vTKNo%'";
		$vReturnArr = array();
		$lvResult = db_query($lvsql);
	...
	function LV_BuilList($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE 1=1  " . $this->GetCondition() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		...
		while ($vrow = db_fetch_array($bResult)) {
			$strL = "";
			$vorder++;
			$vTienPC = $this->LV_GetPCMoney($vrow['lv001'], '331');
			...
			for ($i = 0; $i < count($lstArr); $i++) {
				switch ($lstArr[$i]) {
					...
					case 'lv027':
						if ($vrow['lv027'] == 2) {
							$vrow['lv027'] = $vrow['lv027'] . $vrow['lv030'];
						}
		...
		$strF = $strF . "</tr>";
		$strF = str_replace("<!--lv023-->", $this->FormatView($vSumTienPC, 10), $strF);
	...
	function ListFieldExport($lvFrom, $lvList, $maxRows)
	{
		...
		{
			window.open('" . $this->Dir . "cr_lv0150/?lang=" . $this->lang . "&childfunc='+value+'&ID=" . base64_encode($this->lv002) . "','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReport($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE 1=1  " . $this->RptCondition . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function LV_BuilListReportMini($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvDateSort)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE 1=1 and lv009 like '$lvDateSort%' " . $this->GetConditionMini() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function LV_BuilListReportOther($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE 1=1  " . $this->GetConditionRpt() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	private function sqlcondition($vFile, $vSelectID)
	{
		...
		switch ($vFile) {
			case 'lv027':
				$vsql = "select lv001,lv002,IF(concat(lv001,'')='$vSelectID',1,0) lv003 from  cr_lv0120 order by cr_lv0120.lv003 asc";
				break;
			case 'lv002':
				//$strwh=$this->Get_WHControler();
				//echo $vsql="select A.lv001,A.lv003 lv002,IF(A.lv001='$vSelectID',1,0) lv003 from  ts_lv0001 A inner join wh_lv0034 B on A.lv001=B.lv003 and B.lv002='MINHPHUONG'  where A.lv001 in ('KHOVATTU','3') or A.lv001 like 'KHOVATTU%'";
			...
			case 'lv114':
				if ($this->PlanID == '')
					$vsql = "select lv001,concat(lv004,' ',DATE_FORMAT(lv005,'%d/%m/%Y %H:%i')) lv002,IF(lv001='$vSelectID',1,0) lv003 from  cr_lv0005 where (lv003='ĐNVT' and lv011='1' and lv027=0) or lv001='$vSelectID'";
				else
					$vsql = "select lv001,concat(lv004,' ',DATE_FORMAT(lv005,'%d/%m/%Y %H:%i')) lv002,1 lv003 from  cr_lv0005 where (lv003='ĐNVT' and lv011='1'  and lv027=0 and lv002='$this->PlanID')  or lv001='$vSelectID'";
				break;
		}
		return $vsql;
	}
	public  function getvaluelink($vFile, $vSelectID)
	{
		...
		switch ($vFile) {
			...
			case 'lv117':
				...
				break;
			case 'lv027':
				$vsql = "select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  cr_lv0120 where lv001='$vSelectID'";
				break;
			case 'lv829':
	...
	function MB_LayMACV()
	{
		$vsql = "select lv001,concat(lv004,' ',DATE_FORMAT(lv005,'%d/%m/%Y %H:%i')) lv002 from  cr_lv0005 where (lv003='ĐNVT' and lv011='1' and lv027=0)";
		return db_query($vsql);
	}
	function MB_LayDSKho()
	{
		$vsql = "select * from  ts_lv0001";
		return db_query($vsql);
	}
	function MB_LayDSPhieu()
	{
		$sqlS = "SELECT *,lv114 lv214,lv003 lv829,lv003 lv862 FROM cr_lv0150 WHERE 1=1  " . $this->GetCondition() . "";
		return db_query($sqlS);
	}
	function MB_LayDVT()
	{
		$sqlS = "SELECT lv001, lv002 from sl_lv0005";
		return db_query($sqlS);
	}
}