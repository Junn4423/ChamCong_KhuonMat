<?php
/////////////coding hr_kb0002///////////////
class hr_kb0012 extends lv_controler
{
	public $lv001 = null;
	public $lv002 = null;
	public $lv003 = null;
	...
	public $lv046 = null;
	///////////
	public $DefaultFieldList = "lv199,lv001,lv099,lv015,lv129,lv002,lv003,lv022,lv016,lv017,lv009,lv010,lv030,lv021,lv008,lv039,lv040,lv023";
	////////////////////GetDate
	...
	var $ArrPush = array();
	var $ArrFunc = array();
	var $ArrGet = array("lv001" => "2", "lv002" => "3", "lv003" => "4", "lv004" => "5", "lv005" => "6", "lv006" => "7", "lv007" => "8", "lv008" => "9", "lv009" => "10", "lv010" => "11", "lv011" => "12", "lv012" => "13", "lv013" => "14", "lv014" => "15", "lv015" => "16", "lv016" => "17", "lv017" => "18", "lv018" => "19", "lv019" => "20", "lv020" => "21", "lv021" => "22", "lv022" => "23", "lv023" => "24", "lv024" => "25", "lv025" => "26", "lv099" => "100", "lv129" => "130", "lv030" => "31", "lv039" => "40", "lv040" => "41", "lv199" => "200");
	var $ArrView = array("lv001" => "0", "lv002" => "0", "lv003" => "0", "lv004" => "0", "lv005" => "4", "lv006" => "0", "lv007" => "4", "lv008" => "0", "lv009" => "0", "lv010" => "0", "lv011" => "0", "lv012" => "0", "lv013" => "0", "lv014" => "0", "lv015" => "0", "lv016" => "4", "lv017" => "4", "lv018" => "4", "lv019" => "4", "lv020" => "0", "lv021" => "0", "lv022" => "0", "lv023" => "4", "lv024" => "2", "lv025" => "0", "lv039" => "0", "lv040" => "0", "lv099" => "0", "lv030" => "0", "lv199" => "0");
	...
	function LV_SendMailAgain($lvarr)
	{
		if ($this->isApr == 0) return false;
		$vNoiDungGui = "";
		$stt = 0;
		$vProcess = false;
		$lvsql = "select lv001,TIME_TO_SEC(TIMEDIFF(concat(CURDATE(),' ',CURTIME()),lv025)) HCreate,TIME_TO_SEC(TIMEDIFF(concat(CURDATE(),' ',CURTIME()),lv027)) HCreate1,lv031,lv032,lv033,lv034,lv035,lv036,lv037,lv038 from hr_kb0002 where hr_kb0002.lv001 IN ($lvarr)  ";
		$vReturn = db_query($lvsql);
		while ($vrow = db_fetch_array($vReturn)) {
			$stt++;
			$vNoiDungGui = $vNoiDungGui . "<tr><td>" . $stt . "</td><td>" . $vrow['lv038'] . "</td><td>" . $vrow['lv036'] . "</td></tr>";
			$vProcess = true;
			$lvemail = "noreply@soitheky.vn";
			$vTo = $vrow['lv033'];
			if ($vTo != "") $this->LV_SendMail($vrow['lv036'], $vrow['lv038'], 'admin', $lvemail, $vTo);
		}
		if ($_SESSION['ERPSOFV2RUserID'] == 'md') {
			if ($vNoiDungGui != '') echo '<table border="1"><tr><td>STT</td><td align=center>Chủ để</td><td align=center>Nội dung</td></tr>' . $vNoiDungGui . '</table>';
		}
		return $vProcess;
	}
	function LV_Aproval($lvarr)
	{
		...
		while ($vrow = db_fetch_array($vReturn)) {
			...
			$lvsql = "update hr_kb0002 set lv021=1,lv019=NOW(),lv028='' where lv001='" . $vrow['lv001'] . "'";
			...
			if ($vReturn1) {
				...
				if ($capDuyet == 3) {
					...
					$id = $lvarr; // Đảm bảo là số/nguyên, không phải chuỗi lạ
	...
	{
		include("ml_lv0008.php");
		include("ml_lv0100.php");
		include("ml_lv0009.php");
		$lvListId_del = "";
	...
	{
		if ($this->isApr == 0) return false;
		$lvsql = "Update hr_kb0002 set lv006='1',lv018=NOW()  WHERE hr_kb0002.lv001 IN ($lvarr)  ";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
			$this->InsertLogOperation($this->DateCurrent, 'hr_kb0002.approval', sof_escape_string($lvsql));
		}
	...
	{
		if ($this->isApr == 0) return false;
		$lvsql = "Update hr_kb0002 set lv021='1',lv019=NOW()  WHERE hr_kb0002.lv001 IN ($lvarr)  ";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	protected function GetCondition()
	{
		...
		if ($this->lv046 != "") $strCondi = $strCondi . " and lv046 = '$this->lv046'";
	...
	function LV_BuilList($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		echo $sqlS = "SELECT *,lv015 lv099,lv015 lv129 FROM hr_kb0002 WHERE 1=1  " . $this->GetCondition() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
...
";
	...
	function LV_BuilListReport($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $vSortNum)
	{
		...
		$sqlS = "SELECT *,lv015 lv099,lv015 lv129 FROM hr_kb0002 WHERE 1=1  " . $this->RptCondition . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function getvaluelink($vFile, $vSelectID)
	{
		switch ($vFile) {
			...
			case 'lv129':
				$lvopt = 0;
				$vsql = "select B.lv001,B.lv003 lv002 from  hr_lv0020 A inner join hr_lv0002 B on A.lv029=B.lv001 where A.lv001='$vSelectID'";
				break;
			default:
				$vsql = "";
				break;
