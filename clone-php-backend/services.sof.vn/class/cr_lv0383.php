<?php
/////////////coding cr_lv0383///////////////
class   cr_lv0383 extends lv_controler
{
	public $lv001 = null;
	public $lv002 = null;
	public $lv003 = null;
	...
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update cr_lv0150 set lv030=1  WHERE cr_lv0150.lv001 IN ($lvarr)  and lv007=1 and lv027=2 and lv030=0";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	function LV_SetHistory($vFun, $vLongTermID)
	{
		...
		switch ($vFun) {
			case 'Apr':
				$vTitle = "ĐN cấp VT trợ lý duyệt!";
				break;
			case 'UnApr':
				$vTitle = "ĐN cấp VT trợ lý trả lại!";
				break;
			default:
				break;
		}
		//cr_lv0313
		if ($vTitle != '') {
			$lvsql = "insert into cr_lv0313 (lv002,lv003,lv004,lv005,lv006,lv007,lv009) values('" . $vLongTermID . "','" . sof_escape_string($vTitle) . "','$this->LV_UserID',now(),'$vFun',2,'" . sof_escape_string($this->Remark) . "')";
			$vReturn = db_query($lvsql);
			if ($vReturn) {
	...
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update cr_lv0150 set lv007=0,lv027=0,lv028=0  WHERE cr_lv0150.lv001 IN ($lvarr) and lv007=1 and lv027=2 ";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
	...
	protected function GetCondition()
	{
		...
		if ($this->isAll == 0) {
			$strCondi = $strCondi . " and B.lv042='" . $this->mlv_lv0007->lv904 . "'";
		}
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM cr_lv0150 A inner join hr_lv0020 B on A.lv003=B.lv001 WHERE A.lv007=1 and A.lv027=2 and A.lv030=0 " . $this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	...
	function LV_BuilList($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,A.lv114 lv214,A.lv003 lv829,A.lv003 lv862 FROM cr_lv0150 A inner join hr_lv0020 B on A.lv003=B.lv001 WHERE A.lv007=1 and A.lv027=2 and A.lv030=0  " . $this->GetCondition() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
	...
	function ListFieldExport($lvFrom, $lvList, $maxRows)
	{
		...
		{
			window.open('" . $this->Dir . "cr_lv0383/?lang=" . $this->lang . "&childfunc='+value+'&ID=" . base64_encode($this->lv002) . "','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReport($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.*,A.lv114 lv214,A.lv003 lv829,A.lv003 lv862 FROM cr_lv0150 A inner join hr_lv0020 B on A.lv003=B.lv001 WHERE A.lv007=1 and A.lv027=2 and A.lv030=0  " . $this->RptCondition . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function LV_BuilListReportMini($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvDateSort)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.*,A.lv114 lv214,A.lv003 lv829,A.lv003 lv862 FROM cr_lv0150 A inner join hr_lv0020 B on A.lv003=B.lv001 WHERE A.lv007=1 and A.lv027=2 and A.lv030=0 and lv009 like '$lvDateSort%' " . $this->GetConditionMini() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
	...
	function LV_BuilListReportOther($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList)
	{
		...
		$lvTdH = "<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd = "<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.*,A.lv114 lv214,A.lv003 lv829,A.lv003 lv862 FROM cr_lv0150 A inner join hr_lv0020 B on A.lv003=B.lv001 WHERE A.lv007=1 and A.lv027=2 and A.lv030=0  " . $this->GetConditionRpt() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
		$this->Count = db_num_rows($bResult);
		$strTrH = "";
