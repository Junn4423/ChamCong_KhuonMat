...
class cr_lv0052 extends lv_controler
{
	...
	public $lv004 = null;
	//////////
	public $DefaultFieldList = "lv199,lv001,lv115,lv009,lv008,lv005,lv025,lv026,lv028,lv029,lv030,lv031,lv018,lv020,lv021,lv006,lv007,lv027,lv015,lv016,lv017,lv022,lv023,lv024,lv019,lv010,lv011,lv012,lv013,lv014,lv003,lv004";
	////////////////////GetDate
	...
	var $ArrPush = array();
	var $ArrFunc = array();
	var $ArrGet = array("lv001" => "2", "lv002" => "3", "lv003" => "4", "lv004" => "5", "lv005" => "6", "lv006" => "7", "lv007" => "8", "lv008" => "9", "lv009" => "10", "lv010" => "11", "lv011" => "12", "lv012" => "13", "lv013" => "14", "lv014" => "15", "lv015" => "16", "lv016" => "17", "lv017" => "18", "lv018" => "19", "lv019" => "20", "lv020" => "21", "lv021" => "22", "lv022" => "23", "lv023" => "24", "lv024" => "25", "lv025" => "26", "lv026" => "27", "lv027" => "28", "lv028" => "29", "lv029" => "30", "lv030" => "31", "lv031" => "32", "lv115" => "116", "lv199" => "200", "lv608" => "609", "lv609" => "610", "lv610" => "611", "lv612" => "613", "lv649" => "650", "lv652" => "653", "lv199" => "200");
	var $ArrView = array("lv001" => "0", "lv002" => "0", "lv003" => "0", "lv004" => "0", "lv005" => "0", "lv006" => "2", "lv007" => "2", "lv008" => "10", "lv009" => "0", "lv010" => "0", "lv011" => "0", "lv012" => "0", "lv013" => "0", "lv014" => "0", "lv015" => "0", "lv016" => "0", "lv017" => "0", "lv018" => "0", "lv019" => "0", "lv020" => "0", "lv021" => "0", "lv022" => "0", "lv023" => "0", "lv024" => "0", "lv025" => "0", "lv026" => "0", "lv027" => "2", "lv028" => "0", "lv029" => "0", "lv030" => "0", "lv031" => "0");
	var $ArrViewEnter = array("lv612" => "-1", "lv609" => "-1", "lv610" => "-1", "lv649" => "-1", "lv199" => "-1", "lv001" => "-1", "lv002" => "99", "lv003" => "-1", "lv004" => "-1", "lv005" => "5", "lv006" => "2", "lv007" => "2", "lv009" => "33", "lv010" => "-1", "lv011" => "-1", "lv012" => "-1", "lv013" => "-1", "lv014" => "-1", 'lv018' => "999", "lv115" => "-1", "lv026" => "33", "lv027" => "2", "lv028" => "33", "lv029" => "33", "lv025" => "999");
	...
	function LV_SetTypeBH()
	{
		if ($this->isTypeBH == 1) {
			switch ($this->TypeBH) {
				case 2:
					$this->DefaultFieldList = "lv199,lv015,lv016,lv017,lv612,lv609,lv610,lv649,lv008,lv652,lv028,lv029,lv030,lv031,lv013,lv014";
					break;
				case 1:
					$this->DefaultFieldList = "lv199,lv015,lv016,lv017,lv006,lv018,lv019,lv008,lv025,lv026,lv027,lv020,lv021,lv022,lv023,lv007,lv024,lv013,lv014";
					break;
				default:
					$this->DefaultFieldList = "lv199,lv015,lv016,lv017,lv612,lv609,lv610,lv649,lv008,lv652,lv025,lv026,lv027,lv031,lv013,lv014";
					break;
			}
		} else {
	...
	function LV_Insert()
	{
		...
		$this->lv027 = ($this->lv027 != "") ? recoverdate(($this->lv027), $this->lang) : $this->DateDefault;
		//////////////PBH
		...
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'cr_lv0052.insert', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_InsertAuto()
	{
		...
		$vReturn = db_query($lvsql);
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'sl_lv0009.insert', sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_GetTextBaoHanh($vNoiDung)
	{
		$vArrReturn = array();
		if (trim($vNoiDung) == '')
			return $vArrReturn;
		$vArr = explode(' ', $vNoiDung);
		foreach ($vArr as $vgiatri) {
			if (trim($vgiatri) != '') {
				if (is_numeric($vgiatri)) {
					$vArrReturn[0] = $vgiatri;
				} else {
					$vTrave = $this->LV_DoNgayThangNam($vgiatri);
					if ($vTrave != '')
						$vArrReturn[1] = $vTrave;
				}
			}
	...
	function LV_DoNgayThangNam($vTenCheck)
	{
		$vTenCheck = trim($vTenCheck);
		$vTenCheck = mb_strtoupper($vTenCheck, 'UTF-8');
		if ($this->IsArrDuLieu != true) {
			$this->LV_GetDataLanDau();
		}
		foreach ($this->ArrDuLieu as $key => $vValue) {
			if ($vTenCheck == $key) {
				return $vValue;
			}
		}
	...
	function LV_GetDataLanDau()
	{
		$this->IsArrDuLieu = true;
		$lvsql = "select lv001,lv002 from cr_lv0334 ";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
			$vrow['lv002'] = trim($vrow['lv002']);
			$vVa = mb_strtoupper($vrow['lv002'], 'UTF-8');
			$this->ArrDuLieu[$vVa] = $vrow['lv001'];
		}
	...
	function LV_AutoInsertUpdateGroup($vPBHID, $vMaKHID, $vDateFrom)
	{
		...
		$lvsql = "select A.lv001 from cr_lv0330 A where A.lv115='$vPBHID' and A.lv002='$vMaKHID' and A.lv006=0";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
			return $vrow['lv001'];
		}
		if ($vVRun) {
			...
			$vDateFrom = $this->mosl_lv0013->lv353;
			$vDateTo = $this->mosl_lv0013->lv354;
			$vlv316 = $this->mosl_lv0013->lv316;
			$vlv318 = $this->mosl_lv0013->lv318;
			$vMaGPHM = InsertWithCheckFist('cr_lv0330', 'lv001', '/BBNBH/MP' . substr(getyear($this->DateCurrent), -2, 2), 4);
			$vMaCV = '';
			$lvsql = "insert into cr_lv0330 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv114,lv115,lv353,lv316,lv318) values('$vMaGPHM','$vMaKHID','','$vDateFrom','$vDateTo','0','$this->lv007','$this->lv008','$this->lv009','$this->LV_UserID',now(),'$vJobIDRun','$vPBHID',CURDATE(),'$vlv316','$vlv318')";
			$vReturn = db_query($lvsql);
			if ($vReturn) {
				if ($vReturn)
				...
				//Bảo hành vỏ
				$vlv022 = $this->mosl_lv0013->lv022;
				$vArrBH = $this->LV_GetTextBaoHanh($vlv022);
				$vlv360 = 0;
				$vlv361 = '';
				if ($vArrBH[0] != null)
					$vlv360 = $vArrBH[0];
				...
				//Thời hạn BH Bộ đèn
				//Thời hạn BH Pin
				$vlv117 = $this->mosl_lv0013->lv117;
				$vArrBH = $this->LV_GetTextBaoHanh($vlv117);
				$vlv364 = 0;
				$vlv365 = '';
				if ($vArrBH[0] != null)
					$vlv364 = $vArrBH[0];
				if ($vArrBH[1] != null)
					$vlv365 = $vArrBH[1];
				//Thời hạn BH (Bóng Trang Trí)
				$vlv106 = $this->mosl_lv0013->lv106;
				$vArrBH = $this->LV_GetTextBaoHanh($vlv106);
				$vlv368 = 0;
				$vlv369 = '';
				if ($vArrBH[0] != null)
					$vlv368 = $vArrBH[0];
				if ($vArrBH[1] != null)
					$vlv369 = $vArrBH[1];
				//Ngày bảo hành PBH	
				$vlv353 = $this->mosl_lv0013->lv019;
				$vsql = "update cr_lv0330 t1  set t1.lv360='$vlv360',t1.lv361='$vlv361',t1.lv364='$vlv364',t1.lv365='$vlv365',t1.lv368='$vlv368',t1.lv369='$vlv369',t1.lv353='$vlv353' where t1.lv001='$vMaGPHM'";
				$vReturn1 = db_query($vsql);
				if ($vReturn1) {
					if ($vReturn1)
						$this->InsertLogOperation($this->DateCurrent, 'cr_lv0330.update', sof_escape_string($vsql));
				}
				return $vMaGPHM;
			} else {
				...
				if ($vReturn) {
					...
					//Ngày bảo hành PBH	
					//$vlv353=$this->mosl_lv0013->lv019;
					//Thời hạn BH Bóng (Tube/E27/MR16/GUS.3/)
					$vlv370 = $vlv368;
					$vlv371 = $vlv369;
					$vsql = "update cr_lv0330 t1  set t1.lv360='$vlv360',t1.lv361='$vlv361',t1.lv364='$vlv364',t1.lv365='$vlv365',t1.lv368='$vlv368',t1.lv369='$vlv369',t1.lv370='$vlv370',t1.lv371='$vlv371' where t1.lv001='$vMaGPHM'";
					$vReturn1 = db_query($vsql);
					if ($vReturn1) {
						if ($vReturn1)
							$this->InsertLogOperation($this->DateCurrent, 'cr_lv0330.update', sof_escape_string($vsql));
	...
	function LV_UpdateSQL($lvsql)
	{
		...
		$vReturn = db_query($lvsql);
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'cr_lv0052.update', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update()
	{
	...
	function LV_Delete($lvarr)
	{
		...
		$vReturn = db_query($lvsql);
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'cr_lv0052.delete', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($lvarr)
	{
		...
		if ($vSTT > 0)
			$this->LV_XuLyCanhBao($this->ChildBaoHanhID, $this->mocr_lv0049->lv004);
		return $vReturn;
	}
	function LV_XuLyCanhBao($vCodeID, $vSL)
	{
		$lvsql = "select sum(lv008) TongSL from cr_lv0052  WHERE lv002='$vCodeID'  ";
		$vresult = db_query($lvsql);
		while ($vrow = db_fetch_array($vresult)) {
			$vPercent = $vrow['TongSL'] * 100 / $vSL;
			if ($vPercent > 10) {
				$this->mosl_lv0013->LV_LoadID($this->mocr_lv0049->lv002);
				$this->mocr_lv0005->LV_LoadID($this->mosl_lv0013->lv114);
				$this->mocr_lv0051->lv008 = 7;
				$this->mocr_lv0051->lv006 = $this->mocr_lv0005->FormatView(ADDDATE($this->mocr_lv0049->lv014, -$this->mocr_lv0049->lv008), 2);
				$this->mocr_lv0051->lv005 = $vCodeID;
				$this->mocr_lv0051->lv007 = $this->mocr_lv0005->FormatView($this->mocr_lv0049->lv014, 2);
				$this->mocr_lv0051->lv002 = $this->mocr_lv0005->lv002;
				$this->mocr_lv0051->lv003 = $this->mocr_lv0005->lv001;
				$this->mocr_lv0051->lv004 = 'BHPBH';
				$this->mocr_lv0051->lv009 = 'Cảnh báo SL[' . $vrow['TongSL'] . '] bảo hành ' . round($vPercent, 0) . '% lớn hơn 10% của tổng SL[' . $vSL . '] bán!';
				$this->mocr_lv0051->LV_Insert();
			}
		}
	}
	...
	protected function GetCondition()
	{
		...
		switch ($this->datetype) {
			case 0:
				if ($this->datefrom != '') {
					$strCondi = $strCondi . " and lv006 >= '" . recoverdate($this->datefrom, $this->lang) . "'";
				}
				if ($this->dateto != '') {
					$strCondi = $strCondi . " and lv006 <= '" . recoverdate($this->dateto, $this->lang) . "'";
				}
				break;
			case 1:
				if ($this->datefrom != '') {
					$strCondi = $strCondi . " and lv007 >= '" . recoverdate($this->datefrom, $this->lang) . "'";
				}
				if ($this->dateto != '') {
					$strCondi = $strCondi . " and lv007 <= '" . recoverdate($this->dateto, $this->lang) . "'";
				}
				break;
		}
		if ($this->lv001 != "") {
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM cr_lv0052 WHERE 1=1 " . $this->GetCondition();
	...
	function LV_BuilList($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		$lvTd = "<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM cr_lv0052 WHERE 1=1  " . $this->GetCondition() . " $strSort LIMIT $curRow, $maxRows";
		$vorder = $curRow;
		$bResult = db_query($sqlS);
	...
	function LV_BuilListReportOtherMP($lvList, $lvFrom, $lvChkAll, $lvChk, $curRow, $maxRows, $paging, $lvOrderList, $lvSortNum)
	{
		...
		";
		$lvTrH = "<tr>
			<td style=\"border-top: 1px solid #000000;border-left: 1px solid #000000;font-size16: 16px; font-family:arial, times new roman;\" width=1% valign=\"center\"><strong>" . $this->ArrPush[1] . "</strong></td>	
			@#01
		</tr>
		";
		$lvTr = "<tr >
		...
		";
		$lvTdH = "<td  style=\"border-top: 1px solid #000000;border-left: 1px solid #000000;font-size16: 16px; font-family:arial, times new roman;\" width=\"@01\" align=\"center\" valign=\"center\"><strong>@02</trong></td>";
		$lvTdHE = "<td  style=\"border-top: 1px solid #000000;border-left: 1px solid #000000;border-right: 1px solid #000000;font-size16: 16px; font-family:arial, times new roman;\" width=\"@01\" align=\"center\" valign=\"center\"><strong>@02</trong></td>";
		...
		if ($this->GroupProduct == 1) {
			$sqlS = "SELECT * FROM cr_lv0052 WHERE lv115='$this->lv115'";
		} else {
			$sqlS = "SELECT * FROM cr_lv0052 WHERE lv115='$this->lv115'";
		}
		$vorder = $curRow;
...
";
	...
	function getChiTiet($lv001)
	{
		$vArrRe = [];
		$vsql = "SELECT * FROM `cr_lv0052` WHERE lv115 = '$lv001'";
		$vresult = db_query($vsql);
	...
	function xoa($lv001)
	{
		$success = true;
		$errorMessage = '';
		$vsql = "DELETE FROM `cr_lv0052` WHERE lv001 = '$lv001'";
