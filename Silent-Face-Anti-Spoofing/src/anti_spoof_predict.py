...
class Detection:
    ...
    def get_bbox(self, img):
        ...
        # Check if out is 1D (single detection or empty) or 2D
        if len(out.shape) == 1:
             # Handle case where output might not be as expected or single detection
             if out.shape[0] > 2 and out[2] > self.detector_confidence:
                 left, top, right, bottom = out[3]*width, out[4]*height, out[5]*width, out[6]*height
