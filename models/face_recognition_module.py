...
class FaceRecognition:
    def __init__(self, det_thresh=0.5, det_size=None):
        ...
        det_size: Tuple (width, height) or None to use PERFORMANCE_CONFIG defaults.
        """
        self.known_face_encodings = []
        self.known_face_names = []
        self.known_face_ids = []
        # Initialize InsightFace Wrapper with custom params (uses config if det_size is None)
    ...
    def encode_face_from_image(self, image_file):
        ...
        if len(results) > 1:
            ...
            print(f"⚠️ Tìm thấy {len(results)} khuôn mặt, lấy khuôn mặt lớn nhất.")
    ...
    def encode_face_from_base64(self, base64_string):
        ...
        try:
            ...
            if image is None:
                return None, "Không thể giải mã ảnh base64"
    ...
    def compute_distance(self, face_encodings, face_to_compare):
        """
        Compute distance between face_encodings and face_to_compare.
        Using Cosine Distance (1 - Cosine Similarity).
        Assumes face_encodings are already normalized.
        """
        if len(face_encodings) == 0:
            return np.empty((0))
            
        # face_encodings is assumed to be normalized
        # face_to_compare needs normalization
    ...
    def compare_faces(self, known_face_encodings, face_encoding_to_check, tolerance=0.6):
        ...
        Tolerance: 0.6 is typical for ArcFace (cosine distance).
        """
    ...
    def recognize_face_from_frame(self, frame):
        ...
        Returns: face_locations, face_names, face_ids, face_encodings, min_distances
        """
        # InsightFace detect + encode
        ...
        for res in results:
            ...
            # Convert bbox to (top, right, bottom, left) for consistency if needed, 
            # but we can just use [top, right, bottom, left] format for drawing.
            # Wait, dlib uses (top, right, bottom, left).
            # bbox is [x1, y1, x2, y2] -> (y1, x2, y2, x1)
            face_loc = (bbox[1], bbox[2], bbox[3], bbox[0])
            face_locations.append(face_loc)
            face_encodings.append(embedding)
            
            # Recognition logic (Cosine Similarity)
            name = "Unknown"
            user_id = None
            min_dist = 0.0 # Similarity score (higher is better)
            
            if len(self.known_face_encodings) > 0:
                # Optimized recognition using pre-normalized encodings
                # self.known_face_encodings is already normalized
                ...
                # Threshold for ArcFace is usually around 0.3 - 0.5 depending on model.
                # Buffalo_s is quite strict. Let's try 0.4.
                if best_sim > 0.4:
                    name = self.known_face_names[best_match_index]
                    user_id = self.known_face_ids[best_match_index]
                    min_dist = 1.0 - best_sim # Convert to "distance" for compatibility (lower is better)
                else:
    ...
    def draw_faces_on_frame(self, frame, face_locations, face_names):
        """Vẽ khung và tên lên frame với font Unicode (Pillow)."""
        # Convert BGR to RGB
