# Matplotlib strictly removed for PyInstaller build
import cv2
import numpy as np
import insightface
...
class OnnxAntiSpoofingWrapper:
    def __init__(self, model_dir='Silent-Face-Anti-Spoofing/resources/anti_spoof_models'):
        ...
        providers = [provider for provider in ('CPUExecutionProvider',) if provider in available_providers]
        if not providers:
            providers = ['CPUExecutionProvider']  # Fallback to default CPU provider
...
class AntiSpoofingWrapper:
    def __init__(self, model_dir='Silent-Face-Anti-Spoofing/resources/anti_spoof_models', device_id=0):
        ...
        self.use_onnx = INTEL_OPTIMIZATION['use_onnx_antispoof']
        self.backend = None
...
class InsightFaceWrapper:
    def __init__(self, device='cpu', det_thresh=0.5, det_size=None):
        """
        Initialize InsightFace with Intel CPU optimizations.
        det_size: Tuple (width, height) or None to use config defaults.
                  Smaller sizes = faster but may miss distant faces.
                  640x640: Fast, good for close range
                  960x960: Balanced
                  1280x1280: Slow, good for long range
        """
        # Use config values if det_size not explicitly provided
        if det_size is None:
            det_size = (
        ...
        # Use OpenVINO if available (best for Intel CPUs), otherwise CPU
        ...
        print(f"  OpenVINO cache: {INTEL_OPTIMIZATION['openvino_cache_dir']}")
        ...
        # For YuNet (if we want to use it explicitly as requested)
        # self.detector = cv2.FaceDetectorYN.create(...) 
        # But let's try InsightFace's default first as it's cleaner. 
        # Wait, user explicitly said "Thay RetinaFace bằng YuNet hoặc SCRFD". 
        # buffalo_s uses SCRFD. So using FaceAnalysis satisfies "SCRFD".
        
    def detect_and_encode(self, frame):
        """
        Detect faces and return embeddings.
        Returns: list of dicts with 'bbox', 'embedding', 'kps'
        """
        faces = self.app.get(frame)
        results = []
        for face in faces:
            # face.bbox is [x1, y1, x2, y2]
            # face.embedding is (512,)
            results.append({
